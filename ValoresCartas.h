#ifndef VALORESCARTAS_H
#define VALORESCARTAS_H

//Primer valor: Ruta de la textura
//Segundo valor: Poder de la Carta
//Tercer valor: Tipo de palo
//Cuarto valor: Numero de la carta

#define espada 1
#define basto 2
#define oro 3
#define copa 4


//
#define CARTA1 "./images/espada1.png", 1, espada, 1
#define CARTA2 "./images/basto1.png", 2, basto, 1
//
#define CARTA3 "./images/espada7.png", 3, espada, 7
#define CARTA4 "./images/oro7.png", 4, oro, 7
//
#define CARTA5 "./images/espada3.png", 5, espada, 3
#define CARTA6 "./images/basto3.png", 5, basto, 3
#define CARTA7 "./images/oro3.png", 5, oro, 3   
#define CARTA8 "./images/copa7.png", 5, copa, 3   
//
#define CARTA9 "./images/espada2.png", 6, espada, 2
#define CARTA10 "./images/basto2.png", 6, basto, 2
#define CARTA11 "./images/oro2.png", 6, oro, 2   
#define CARTA12 "./images/copa2.png", 6, copa, 2
//
#define CARTA13 "./images/copa1.png", 7, copa, 1
#define CARTA14 "./images/oro1.png", 7, oro, 1
//
#define CARTA15 "./images/espada12.png", 8, espada, 12
#define CARTA16 "./images/basto12.png", 8, basto, 12
#define CARTA17 "./images/oro12.png", 8, oro, 12   
#define CARTA18 "./images/copa12.png", 8, copa, 12
//
#define CARTA19 "./images/espada11.png", 9, espada, 11
#define CARTA20 "./images/basto11.png", 9, basto, 11
#define CARTA21 "./images/oro11.png", 9, oro, 11   
#define CARTA22 "./images/copa11.png", 9, copa, 11
//
#define CARTA23 "./images/espada10.png", 10, espada, 10
#define CARTA24 "./images/basto10.png", 10, basto, 10
#define CARTA25 "./images/oro10.png", 10, oro, 10   
#define CARTA26 "./images/copa10.png", 10, copa, 10
//

#define CARTA27 "./images/basto7.png", 11, basto, 7
#define CARTA28 "./images/copa7.png", 11, copa, 7

#define CARTA29 "./images/espada6.png", 12, espada, 6
#define CARTA30 "./images/basto6.png", 12, basto, 6
#define CARTA31 "./images/oro6.png", 12, oro, 6   
#define CARTA32 "./images/copa6.png", 12, copa, 6
//
#define CARTA33 "./images/espada5.png", 13, espada, 5
#define CARTA34 "./images/basto5.png", 13, basto, 5
#define CARTA35 "./images/oro5.png", 13, oro, 5   
#define CARTA36 "./images/copa5.png", 13, copa, 5
//
#define CARTA37 "./images/espada4.png", 14, espada, 4
#define CARTA38 "./images/basto4.png", 14, basto, 4
#define CARTA39 "./images/oro4.png", 14, oro, 4   
#define CARTA40 "./images/copa4.png", 14, copa, 4

#define TODAS Carta(CARTA1), Carta(CARTA2), Carta(CARTA3), Carta(CARTA4), Carta(CARTA5), Carta(CARTA6), Carta(CARTA7), Carta(CARTA8), Carta(CARTA9), Carta(CARTA10), Carta(CARTA11), Carta(CARTA12), Carta(CARTA13), Carta(CARTA14), Carta(CARTA15), Carta(CARTA16), Carta(CARTA17), Carta(CARTA18), Carta(CARTA19), Carta(CARTA20), Carta(CARTA21), Carta(CARTA22), Carta(CARTA23), Carta(CARTA24), Carta(CARTA25), Carta(CARTA26), Carta(CARTA27), Carta(CARTA28), Carta(CARTA29), Carta(CARTA30), Carta(CARTA31), Carta(CARTA32), Carta(CARTA33), Carta(CARTA34), Carta(CARTA35), Carta(CARTA36), Carta(CARTA37), Carta(CARTA38), Carta(CARTA39), Carta(CARTA40) 
	

#endif
